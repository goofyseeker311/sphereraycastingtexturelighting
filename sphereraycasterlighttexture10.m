...
function [k,n,v,u] = raysphereintersection(sphere, ray)
  ...
  nmatrix = [(kmatrix(:,1).*ray(:,4)).+ray(:,1) (kmatrix(:,1).*ray(:,5)).+ray(:,2) (kmatrix(:,1).*ray(:,6)).+ray(:,3)]; uvi=find(kmatrix(:,2)>0);uvn=kmatrix(uvi,2);uvp=nmatrix(uvi,:);
  ...
  umatrix(uvi,1)=0.5+ratio.*(acosd(dot([vmatrix(uvi,1:2)],ones(uvicount,1)*[1 0],2)).*((-1).^(vmatrix(uvi,2)<0))); umatrix(uvi,2)=1-(0.5+ratio2.*asind(vmatrix(uvi,3)));
  k = kmatrix; n = nmatrix; v=vmatrix; u = umatrix;
endfunction

wimagefiles={"physical-world-map.jpg","Fondvulcain.jpg","world116cE.jpg"};
imagefile=wimagefiles{3}; wimage=double(imread(imagefile))/255; wims=size(wimage); figure(5); imshow(wimage);
lightvalue=3.5; pointlightscount = 5; pointlights = [100*(rand(pointlightscount,3)-0.5) randi(255,pointlightscount,3)/255];
nspheres = 100; scenespheres = [100*(rand(nspheres,3)-0.5) 6*rand(nspheres,1)+4 ones(nspheres,3)];
...
[k, kn, kv, ku] = raysphereintersection(scenespheres(:,1:4), trays); kd=k(:,1);kb=k(:,2); m = max(kd);
...
plnv=plv./pld3; lv=[knc.+(0.00000001.*plnv) plnv]; [ck, ckn, ckuv] = raysphereintersection(scenespheres(:,1:4), lv); cki=find(ck(:,2)==0); ckicount=size(cki,1); ckni=ckn(cki,:); kcni=kci(cki,:); pld3n=pld3(cki,:);
